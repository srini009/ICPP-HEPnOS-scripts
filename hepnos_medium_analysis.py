...
import hepnos_medium_regression as regressionmodel
from collections import defaultdict, OrderedDict
import glob, re

def populate_pidlist():
	files = glob.glob(str(os.getcwd())+"/sdskv*") #Read all files in data_directory
	pidset = set()
	for f in files:
...
def analyze():
	...
	for pid in pidlist:
		...
		for line in range(0, len(contents3)):
			...
			if acc_pc < 30.0 and acc_pc > -30.0:
				total_error += 1
		...
		while i < len(contents5):
			...
			latency_ratio = (total_latency/ideal_width[0])
